  ...
  <div class="app-container">
    ...
    <div class="content-container">
      <CrudOperation
        ...
        @toSynchronizeUser="handleSynchronizeUser()"
      />
...
import { baseAPI, identityAPI, API } from '@/api/generalAPI'
...
import { createFormList, updateFormList, formList, queryItems, UserCrud, BtnItems, ShelfForm } from './config'
export default {
  name: 'User',
  components: { Table, CrudOperation, EditForm, rePassword, TBShelf },
  ...
  methods: {
    ...
    async submitTBD() {
      // /api/business/ehr/GetEHRUser
      await API.get('ehr', { starttime: this.ShelfData.starttime, endtime: this.ShelfData.endtime }, 'GetEHRUser')
        .then(res => {
          if (res.success) {
            this.TBDialogVisible = false
            this.$notify({
