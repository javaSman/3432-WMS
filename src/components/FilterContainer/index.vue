<template>
  <div class="search-container">
      ...
      <el-row>
        ...
        <el-col :span="3">
          <el-button-group v-if="highSearchList.length" class="noMaskContainer">
            ...
            <el-button size="mini" type="primary" :icon="arrow" @click="handleSearch" />
          </el-button-group>
          <!-- '搜索' -->
          <el-button v-else size="mini" type="primary" class="noMaskContainer" @click="handleFilter">
            {{ $t('button.search') }}
          </el-button>
          <!-- '重置' -->
          <el-button size="mini" type="info" class="noMaskContainer" @click="reset">
            {{ $t('button.reset') }}
          </el-button>
          <!-- '更多搜索' -->
          <el-button v-if="moreSearch" size="mini" type="info" class="noMaskContainer" @click="show = !show">
            {{ $t('button.moreSearch') }}
          </el-button>
        </el-col>
      </el-row>
    ...
    <div v-show="show" class="manysearch">
      <el-row>
        <el-col>
          <el-form size="mini" :inline="true" :model="moreQuery" @submit.native.prevent>
            <el-form-item v-for="item in moreItems" :key="item.index" :label="item.label">
              <FilterItem
                :item="item"
                :list-query="moreQuery"
                :load-options="moreOptions"
                :filter-width="150"
                @handleFilter="handleFilter"
              />
            </el-form-item>
...
export default {
  ...
  props: {
    ...
    moreItems: { type: Array, default: () => [] }, // 查询条参数
    moreQuery: { type: Object, default: () => {} }, // 查询条件
    hasDownload: { type: Boolean, default: false }, // 是否显示导出按钮
    downloadLoading: { type: Boolean, default: false }, // 导出加载状态
    // show: { type: Boolean, default: false }, // 更多搜索
    moreSearch: { type: Boolean, default: false }, // 更多搜索
    loadOptions: { type: Function, default: () => {} }, // 树形下拉框数据加载方法
    moreOptions: { type: Function, default: () => {} }, // 树形下拉框数据加载方法
