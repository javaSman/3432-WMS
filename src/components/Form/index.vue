  ...
  <div class="search_input">
    ...
    >
      <el-row :gutter="layout.gutter">
        ...
        >
          <el-form-item v-if="!item.isHide" :prop="item.prop">
            <!-- 输入规则提示 -->
            <span slot="label">
            ...
            <el-input
              ...
              @keyup.enter.native="item.keyupenter ? item.keyupenter(formData[item.prop]) : ''"
            ...
            <!-- 下拉单选-两列模型 -->
            <el-select
              v-if="item.type === 'SelectTwo' && !item.isHide"
              ...
              clearable
              filterable
              @change="item.change ? handleSelectChange(item, form[item.prop]) : null"
              @clear="item.clear ? item.clear(formData[item.prop]) : ''"
              @visible-change="$forceUpdate()"
            >
              <el-option
                ...
                :key="index"
                :label="item.optProps ? op[item.optProps.label] : op.label"
                :value="item.optProps ? op[item.optProps.value] : op.value"
              >
                <span style="float: left">{{ item.optProps ? op[item.optProps.label] : op.label }}</span>
                <span
           ...
           >{{ ra.label }}</el-radio-button>
            ...
            <DateRangePart v-if="item.type === 'DateRangePart' && !item.isHide" :item="item" :form-data="formData" />
...
<script>
import DateRangePart from '../FilterCollapse/module/DateRangePart'
...
export default {
  name: 'Form',
  components: { DateRange, DateTimeRange, Treeselect, CascaderTree, DateRangePart },
  props: {
    layout: {
      type: Object,
      default: () => {
