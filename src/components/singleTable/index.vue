  ...
  <div>
    <el-form ref="tableForm" :visible="tableShow" :model="tableForm" :inline="true" class="table-form" size="mini">
      ...
      >
        ...
        <!-- 复选框列 -->
        <!-- <el-table-column v-if="hasSelection" type="selection" :selectable="selectable" width="58px" align="center" /> -->
        <el-table-column align="center" width="58" label="">
          <template slot-scope="scope">
          ...
          <template slot="header" slot-scope="scope">
            ...
            >
              <span @mouseover="onMouseOver(scope.$index)">
                <span ref="tableHeader" style="text-align: center">{{ item.label }}</span>
              </span>
            </el-tooltip>
            <span v-else style="text-align: center">{{ item.label }}</span>
          </template>
          <template slot-scope="scope">
            <!-- 输入框 -->
            <el-form-item
            ...
            >
              <el-input-number
                ...
                :max="item.max == undefined ? scope.row['max'] : item.max"
            ...
            <!-- 开关 -->
            <el-switch
              v-else-if="item.type === 'colType/Switch'"
              v-model="scope.row[item.prop]"
              @change="item.change ? item.change(scope.$index, scope.row[item.prop]) : ''"
            />
...
export default {
  ...
  props: {
    ...
    background: { type: Boolean, default: false }, // 是否为分页按钮添加背景色
    ...
    showSummary: { type: Boolean, default: false }, // 是否要合计行
    summaryMethod: {
      type: Function,
      default: () => {
        return true
  ...
  data() {
    return {
      ...
      hideTableX: false // 合计行存在时隐藏表格body的滚动条标识
    }
  },
  computed: {
    /** 当前页 */
  ...
  mounted() {
    ...
    // 隐藏bodyWrapper滚动条，显示footerWrapper滚动条
    let tableBody = this.$refs.table.$refs.footerWrapper
    if (tableBody) {
      this.hideTableX = true
      tableBody.addEventListener('scroll', () => {
        // 滚动距离
        let scrollLeft = tableBody.scrollLeft
        this.$refs.table.$refs.bodyWrapper.scrollLeft = scrollLeft
      })
    }
  },
  methods: {
    ...
    handleSelectionChange(val) {
      ...
      // 设置单选时，行显示高亮
      if (val.length === 1) {
        this.templateSelection = val[0].id // 单选选中id值
        // this.selection = this.dataList.filter((item) => item.id === val.id)
    ...
    cellClickFun(row, column, cell, event) {
      ...
      this.$emit('cellClickFun', row, column, cell, event)
...
.hide-table-x /deep/ .el-table__body-wrapper {
  overflow-x: hidden !important;
}

.table-form /deep/ .el-table__footer-wrapper {
  overflow-x: auto;
