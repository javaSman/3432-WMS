<template>
  <div style="position: relative">
    <el-badge :value="unread" :hidden="badgeHide" class="col-design-btn">
      <!-- '编辑列' -->
      <el-button type="text" :title="$t('dialogHeader.editColumn')" @click="handelColDesign">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- '重置系统设置' -->
        <el-button size="small" icon="el-icon-refresh-left" style="float: left" @click="RZrecoverCol">
          {{ $t('button.RZrestore') }}
        </el-button>
        <!-- '取消' -->
...
export default {
  ...
  methods: {
    ...
    RZrecoverCol() {
      // '是否恢复至原始设置?','提示'
      this.$confirm(this.$t('dialog.RZcolTips'), this.$t('dialog.tip'), {
        confirmButtonText: this.$t('button.confirm'), // '确认'
        cancelButtonText: this.$t('button.cancel'), // '取消'
        type: 'warning'
      })
        .then(() => {
          console.log(this.colTable[0].templateName)
          API.dataPost('usertabletemplatedetail', { templateName: this.colTable[0].templateName }, 'Delete').then(
            res => {
              this.show = false
