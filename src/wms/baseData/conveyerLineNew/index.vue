...
import { column, detailCol, formList, queryItems, detailtBtnItems, DictDetailCrud, FormItems, DictCrud } from './config'
export default {
  ...
  methods: {
    ...
    getDict() {
      ...
      API.getDict('dict', { name: 'BoxState' }).then(res => {
        this.formList[14].options = res.details
      })
      API.getDict('dict', { name: 'BoxType' }).then(res => {
        this.formList[15].options = res.details // 获取箱子类型
      })
    ...
    getStacker() {
      API.get('device', { IsPage: false, DeviceID: 'stacker' }, 'all').then(res => {
        ...
        this.formList[22].options = res.items
      })
    },
    /** 状态切换 */
    handleHaState() {
      var rows = this.detailSelection
      if (rows.lenght === 0) {
        this.$message({
          message: '请至少选择一项！',
          type: 'warning'
      ...
      if (success) {
        this.$confirm('确定对 ' + list.length + ' 进行状态切换吗', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          API.dataPost('conveyerLine', list, 'LineHaState').then(res => {
            if (res) {
              this.$message({
                message: '切换成功！',
                type: 'success'
