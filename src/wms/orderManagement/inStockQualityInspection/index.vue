<template>
  <div class="components-container">
    ...
    <div class="content-container" style="margin-top: 10px">
      <h4>{{ $t('dialogHeader.QualityInspectionFormDetails') }}</h4>
      <ColDesign
        tid="DetailTable"
...
export default {
  ...
  components: { Table, EditDetailOptDialog, CrudOperation, IssueDocPrintsZKBF, UploadDialog, OutShelves },
  ...
  methods: {
    ...
    getDict() {
      ...
      // })
      API.getDict('dict', { name: 'RecheckOrderStatus' }).then(res => {
    ...
    getDetail(row) {
      ...
      API.getData('recheckorder', this.detailQuery, 'GetDetails').then(res => {
    ...
    async submitOutShelves() {
      ...
      await API.dataPost('recheckorder', params, 'RecheckOrderSale')
        .then(res => {
          if (res.success) {
            this.outShelvesDialogVisible = false
            this.$notify({
