<template>
  <div>
    ...
    <div class="content-container" style="margin-top: 10px">
      ...
      />
      <CrudOperation :button-items="SalesDetailBtn" @toOffShelf="handleOffShelf()" />
...
export default {
  ...
  data() {
    return {
      colName: 'OutStockOrder',
      detailColName: 'OutStockOrderDetail',
  ...
  methods: {
    ...
    getDict() {
      API.getDict('dict', { name: 'outStockStatus' }).then(res => {
        this.SalesItems[1].options = res.details
      })
    },
    handleOutShelves() {
