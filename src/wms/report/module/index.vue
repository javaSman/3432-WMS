  ...
  <div>
    ...
    >
      ...
      <div class="opts" style="margin-bottom: 20px">
        <el-button class="filter-item" size="mini" type="warning" icon="el-icon-download" @click="handletoExport()">
          导出
        </el-button>
        <!-- '删除明细' -->
        <!-- <el-button
...
export default {
  ...
  props: {
    ...
    resultExport: { type: Array, default: () => [] }
  },
  data() {
    return {
  ...
  created() {
    // console.log(this.formData.OutWarehouseCode)
  },
  methods: {
    // 合计行自定义
    getTotal(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (this.totalCol.includes(column.property)) {
      ...
      this.$refs.form.$refs.formList.validate(valid => {
        this.$refs.detailTable.$refs.tableForm.validate(async tableValid => {
          if (valid && tableValid) {
            ...
            if (
              this.formData === 'InStock' &&
              (this.formData.supplierCode === undefined || this.formData.supplierName === undefined)
            ) {
    ...
    handletoExport() {
      ...
      this.exportPast('inventoryreport', this.resultExport, 'ExportStockReport')
    },
    exportPast(name, params, otherExport) {
      this.downloadLoading = true
      let other = otherExport || 'Export'
      API.dataPost(name, params, other)
        .then(res => {
          if (res !== '') {
