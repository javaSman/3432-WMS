...
axios.defaults.baseURL = window.globalConfig.base.ip + ':' + window.globalConfig.base.backend_port
// POST传参序列化
...
axios.interceptors.response.use(
  ...
  err => {
    ...
    if (err.response) {
      ...
      switch (
        err.response.status // 判断后台返回的token值是否失效
      ) {
        case 401:
          Message({
            message: '登录过期，请重新登录！',
...
export default {
  ...
  instancePosts(url, params) {
    ...
    var instance = axios.create({
      baseURL: window.globalConfig.base.ip + ':' + window.globalConfig.base.auth_port
  ...
  getPermissions(url, params) {
    ...
    // console.log(config)
    var instance = axios.create({
      baseURL: window.globalConfig.base.ip + ':' + window.globalConfig.base.identity_port
